...
void buildTree(){
  ...
  for (int i = 2; i <= n; i++){
	...
	if (lastchild[f] == 0)
		...
		int j = lastchild[f];
		nodes[j].rchild = lastchild[f] = i;//please finish the following parts
	}
  }
}
...
void Preorder(Node &root){ //please finish the following parts
	cout << root.data;
	if (root.lchild != 0) Preorder(nodes[root.lchild]);		//remember to check if the root.lchild is not 0, if we don't check, the program will be stuck in an infinite loop
